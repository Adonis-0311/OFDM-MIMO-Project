...
|---|---|---:|---:|---|
| Oversampling/off-grid ablation | Section 6.5.13(a) | ok | 9.75 | `Results/oversampling_offgrid_ablation/summary.md` |
| Kruskal identifiability ablation | Section 6.5.13(b,c) | ok | 3.33 | `Results/kruskal_identifiability_ablation/summary.md` |
| Off-grid mismatch lemma validation | Section 5.1.1 / Phase 3 | ok | 2.89 | `Results/offgrid_mismatch_lemma/summary.md` |

## Key Evidence

- Oversampling/off-grid: at `rho=2`, bounded off-grid improves mean NMSE from `-8.54 dB` to `-55.95 dB` across the three scans.
- Kruskal/target-count: as `L` rises from `2` to `32`, NMSE changes from `-57.36 dB` to `-40.60 dB`, and runtime from `0.0065 s` to `0.1141 s`.
...
- CDL-A/C/D and DeepMIMO Set E cross-scene validation are still pending.
