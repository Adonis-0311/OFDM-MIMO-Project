...
outDir = fullfile(rootDir, '05_results', 'deepmimo_set_e_access_audit');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
candidates = [
    dir(fullfile(rootDir, '2025*'));
    dir(fullfile(rootDir, '90_archive', '2025*'))
];
referenceRoot = '';
for i = 1:numel(candidates)
    candidate = fullfile(candidates(i).folder, candidates(i).name, '参考');
    if exist(fullfile(candidate, 'DeepMIMO-5GNR'), 'dir') || exist(fullfile(candidate, 'DeepMIMO-matlab-master'), 'dir')
        referenceRoot = candidate;
        return;
    end
end
referenceRoot = fullfile(rootDir, '90_archive', '2025_thesis_materials', '参考');
end
