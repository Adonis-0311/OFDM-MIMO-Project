...
Figure~\ref{fig:local_family} combines accuracy with runtime. Median end-to-end times are 8.93 ms for grid, 10.58 ms for quadratic interpolation, 33.55 ms for Newton, and 10.04 ms for Cartesian refinement in the common local-family evaluation. The dedicated projection implementation measures Candan at 11.07 ms and projection-selected Candan at 11.24 ms, a 0.17 ms or 1.57\% increment.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.96\textwidth]{tsp_local_family.pdf}
  \caption{Matched-scene comparison on 1200 identical tensors. The projection-selected Candan point includes the fitted-energy decision. Error bars are 95\% intervals over five seed means.}
...
\begin{figure*}[t]
  ...
  \caption{Controlled separation/near--far results averaged over four SNRs. (a) Grid and (b) Candan per-component all-axis half-bin rates, (c) Candan designated-pair joint-hit rate, and (d) Candan measurement-NMSE gain. Each displayed cell contains 100 test scenes across five seeds.}
  \label{fig:stress}
\end{figure*}
