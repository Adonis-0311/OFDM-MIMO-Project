...
Figure~\ref{fig:system_model} places this separable model in the complete estimator. The same orthonormal FFT supplies both the top-$L$ neighborhoods and the axis triplets; the final fitted-energy comparison returns one set of coordinates, gains, and reconstruction.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.96\textwidth]{tsp_estimator_overview.pdf}
  \caption{End-to-end projection-selected tensor estimator. The separable observation is transformed once; top-$L$ FFT neighborhoods provide $3DL$ center/neighbor samples for axis-wise Candan correction, joint LS estimates the gains, and the higher fitted-subspace energy returns either the refined or grid coordinates with the corresponding reconstruction.}
...
Table~\ref{tab:system_params} collects the synthetic and CDL settings used by every reported comparison.
