...
本轮以 `01_design_and_plan/ISAC_DeepUnfolding_TechnicalDesign_v2.2.md` 为基线。v2.2 的新增硬要求主要集中在三类：
...
| 类别 | 文件/目录 | 可用性 | 说明 |
|---|---|---:|---|
| 论文/答辩材料 | `90_archive/2025_thesis_materials/` | 高 | 已有本科毕设终稿、答辩 PPT、开题/中期/评分表等，可作为写作素材和图表来源；其中含个人材料，不建议直接上传公开仓库。 |
| 技术路线 | `01_design_and_plan/ISAC_DeepUnfolding_TechnicalDesign_v2.2.md` | 高 | 可作为后续论文升级和实验排期的操作基线。 |
| MATLAB 原型 | `90_archive/old_experiment/` 和 `06_paper_and_delivery/paper2_package/` | 中 | 已有 OFDM、CDL-like 信道、LS/ANN/CS-DL 命名模块，但接口未完全闭合，需要重构后才能成为主实验 pipeline。 |
| DeepMIMO 参考 | `90_archive/2025_thesis_materials/参考/DeepMIMO-*` | 中 | 有 MATLAB 版本参考代码和 README，可用于后续 Set E 数据接入；场景数据本体仍需确认是否已经下载。 |
| 已有图片 | `90_archive/old_experiment/method_comparison*.png`, `90_archive/old_experiment/OMP_performance_comparison.png` | 中 | 可作为本科阶段结果展示素材；v2.2 论文中需重新标注实验设置并尽量复跑。 |
| 文献 PDF | `90_archive/2025_thesis_materials/参考/阅读文献/` | 中 | 覆盖 CS、MIMO-OFDM、DL-CS 等基础文献；v2.2 还需要补 2024-2025 SOTA。 |
...
新增 `04_experiments/eval/run_oversampling_offgrid_ablation.m`：

- 对应 v2.2 §6.5.13 的字典过采样敏感性。
- 同时支撑 §5.1.1 Off-Grid Mismatch Lemma 的实验直觉。
- 输出目录：`05_results/oversampling_offgrid_ablation/`
...
新增 `04_experiments/eval/run_kruskal_identifiability_ablation.m`：

- 对应 v2.2 §6.5.13(b,c) 的 Kruskal 可辨识性与目标数退化实验。
- 使用轻量 on-grid 多目标张量与 Tensor-OMP 风格逐次检测，扫描 `L={2,4,8,16,24,32}`。
- 输出目录：`05_results/kruskal_identifiability_ablation/`
- 输出文件：CSV、MAT、PNG、PDF、Markdown summary。
...
新增 `04_experiments/eval/run_offgrid_mismatch_lemma_validation.m`：
...
- 输出目录：`05_results/offgrid_mismatch_lemma/`
- 输出文件：summary CSV、sample CSV、PNG、PDF、Markdown summary。

新增 `04_experiments/eval/run_cdl_profile_generalization_smoke.m`：
...
- 输出目录：`05_results/cdl_profile_generalization_smoke/`
...
- 输出目录：`05_results/deepmimo_set_e_access_audit/`
- 输出文件：CSV、Markdown summary。

新增 `04_experiments/eval/run_all_v2_2_supplemental_experiments.m`：

- 一键复跑当前六组 v2.2 轻量补充实验/审计。
- 生成 `05_results/v2_2_supplemental_manifest.md` 与 `05_results/v2_2_supplemental_manifest.csv`。
...
- `90_archive/2025_thesis_materials/` 中的个人材料、合同、评分表、论文 Word/PDF、外部参考压缩包。
- `.mat` 模型/数据中间件。
- 大型下载数据集。
